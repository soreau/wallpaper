		...
		if (options[i]->type == CompOptionTypeString)
		{
			for (j = 0; j < options[i]->nValue; j++)
			{
				if (stat (options[i]->value[j].s, &st) == 0)
				{
					if (S_ISDIR(st.st_mode))
					{
						wallpaperSearchDirectory(s, options[i]->value[j].s, options, new_options, j, &n, *nElements, wallpaperGetRecursive(s->display));
					}
					else
					{
						compLogMessage ("wallpaper", CompLogLevelInfo,
...
updateBackgrounds (CompScreen *s)
{
    ...
    ws->backgrounds =
	processMultiList (ws->backgrounds, &ws->nBackgrounds, s, 5,
					...
					unsigned int mask)
{
	...
	if (w->vCount)
	    (*s->drawWindowTexture) (w, &back->fillTex,
					     &fA, mask);
}
